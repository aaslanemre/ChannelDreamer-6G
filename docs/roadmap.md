...
| 3 | Multi-modal world model: camera (ResNet), LiDAR (offline PointNet-style tokens), GPS trajectory (MLP) encoders feeding a DreamerV3-style RSSM trained by sequence prediction | Done: encoders, caches and RSSM tested on real Scenario 33; `scripts/train_world_model.py` trains the exogenous model until greedy one-step regret <= reactive on both regimes (met at 19k steps, 340 s; held-out 0.15 / 0.42 dB vs reactive 0.18 / 0.43, see `docs/methodology.md` §7) |
